...
export default function ProductTable({ products }: ProductTableProps) {
  ...
  return (
    <div className="overflow-x-auto mt-12 px-20 shadow-md sm:rounded-lg">
      <div className="grid grid-cols-10 min-w-full border-b border-gray-300 text-center text-sm font-semibold text-gray-900">
        <h3 className="col-span-5 px-3 py-3.5 pl-4 pr-3 text-left sm:pl-0">
          Producto
        </h3>
        <h3 className="px-3 py-3.5">
          Precio
        </h3>
        <h3 className="px-3 py-3.5">
          Disponible
        ...
        <h3 className="col-span-2 px-3 py-3.5">
          Acciones
          <span className="sr-only">Acciones</span>
        </h3>
      </div>
      {products.map((product) => (
        <div key={product.id} className="grid grid-cols-10 min-w-full border-b border-gray-300 text-center text-sm text-gray-500">
          <div className="col-span-5 px-3 py-4 pl-4 pr-3 text-left  sm:pl-0">
            {product.name}
          </div>
          ...
          <div className="px-3 py-4 group">
            <Link href={`/order/${product.category.slug}`} className="py-2 rounded-md hover:underline group-hover:text-fuchsia-700 group-hover:bg-gray-200 group-hover:font-bold group-hover:px-10">
